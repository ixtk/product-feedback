...
export const Sidebar = () => {
  ...
  const featureCountElements = FEATURE_COUNT.map(feature => {
    return (
      <div key={feature.status} className="flex justify-between gap-x-4 items-center mb-1 last-of-type:mb-0">
        ...
        <dd className="text-secondary-800 font-bold">{feature.categoryCount}</dd>
      </div>
    )
  })

  return (
    <div className="absolute right-0 bg-base-300 p-5 w-64 flex flex-col gap-y-5 h-screen">
      <BaseContainer classExtension="flex flex-wrap gap-3">
        {tagElements}
      </BaseContainer>
      <BaseContainer>
